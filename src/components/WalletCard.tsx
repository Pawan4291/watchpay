...
import { Wallet, TrendingUp, TrendingDown, RefreshCw, CheckCircle, ExternalLink, AlertCircle } from 'lucide-react';
...
export function WalletCard() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: 'radial-gradient(circle at 80% 20%, rgba(255,107,0,0.04) 0%, transparent 60%)' }} />
      ...
      <div className="relative p-6">
        ...
        <div className="mb-6">
          ...
          <motion.div key={wallet.balance} initial={{ scale: 1.05, color: '#ff8c00' }} animate={{ scale: 1, color: '#ff6b00' }} transition={{ duration: 0.3 }} className="font-orbitron text-4xl font-bold" style={{ textShadow: '0 0 30px rgba(255,107,0,0.5)' }}>
            {wallet.balance.toFixed(4)}
            <span className="text-lg ml-2" style={{ color: '#ff6b0066' }}>WP</span>
          </motion.div>
          <div className="text-xs mt-1" style={{ color: '#333' }}>Last updated: {new Date(wallet.lastUpdated).toLocaleTimeString()}</div>
        </div>
        ...
        <div className="mb-5 p-3 rounded-lg text-xs" style={{ background: 'rgba(255,107,0,0.04)', border: '1px solid rgba(255,107,0,0.1)', color: '#ff6b0088', lineHeight: 1.6 }}>
          ...
          <strong>WP Points</strong> are 1:1 backed by real UCT held in the WatchPay agent wallet ({WATCHPAY_AGENT_NAMETAG}). Deposit sends real UCT there; withdraw sends real UCT back to your Sphere wallet.
        </div>

        <div className="flex gap-3 mb-4">
          <motion.button onClick={() => setMode(mode === 'deposit' ? null : 'deposit')} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-lg font-orbitron text-xs font-bold tracking-widest" style={{ background: mode === 'deposit' ? 'rgba(255,107,0,0.2)' : 'rgba(255,107,0,0.08)', border: `1px solid ${mode === 'deposit' ? '#ff6b00' : 'rgba(255,107,0,0.2)'}`, color: '#ff6b00' }}>
            <TrendingDown size={14} />
            DEPOSIT
          </motion.button>
          <motion.button onClick={() => setMode(mode === 'withdraw' ? null : 'withdraw')} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-lg font-orbitron text-xs font-bold tracking-widest" style={{ background: mode === 'withdraw' ? 'rgba(255,107,0,0.1)' : 'transparent', border: `1px solid ${mode === 'withdraw' ? '#ff6b0066' : '#2a2a2a'}`, color: mode === 'withdraw' ? '#ff6b00' : '#555' }}>
            <TrendingUp size={14} />
            WITHDRAW
          </motion.button>
        </div>
              ...
              <div className="pt-2 pb-2">
                <div className="p-4 rounded-lg" style={{ background: '#0a0a0a', border: '1px solid #1a1a1a' }}>
                  <div className="text-xs font-orbitron mb-3" style={{ color: '#555', letterSpacing: '0.1em' }}>
                    {mode === 'deposit' ? `Your real Sphere wallet sends UCT to ${WATCHPAY_AGENT_NAMETAG}` : 'Agent sends real UCT back to your real Sphere wallet'}
                  </div>
                  <div className="flex gap-2">
                    ...
                    <motion.button
                      ...
                      style={{ background: loading ? '#1a1a1a' : 'linear-gradient(135deg, #ff6b00, #ff8c00)', color: loading ? '#555' : 'white', border: 'none', cursor: loading ? 'not-allowed' : 'pointer' }}
                    >
                      {loading ? '...' : mode === 'deposit' ? 'SEND' : 'WITHDRAW'}
                    </motion.button>
                  </div>
                  {mode === 'withdraw' && parseFloat(withdrawAmount) > wallet.balance && (
                    <div className="text-xs mt-2" style={{ color: '#ff4444' }}>Insufficient balance</div>
                  )}
                </div>
              </div>
            </motion.div>
          ...
          {lastTx && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mt-3 p-3 rounded-lg" style={{ background: 'rgba(0,255,136,0.05)', border: '1px solid rgba(0,255,136,0.2)' }}>
              ...
              <div className="flex items-center gap-2">
                <span className="tx-hash truncate text-xs" style={{ fontSize: '0.58rem' }}>TX: {lastTx}</span>
                {lastTx !== 'pending confirmation' && (
                  <a href={`https://explorer.unicity.network/tx/${lastTx}`} target="_blank" rel="noopener noreferrer" style={{ color: '#ff6b0066', flexShrink: 0 }}>
